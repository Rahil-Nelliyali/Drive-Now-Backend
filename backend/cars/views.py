...
class CarLocationListView(ListAPIView):
    serializer_class = LocationSerializer

    def get_queryset(self):
        car_id = self.kwargs.get("id")
        return Location.objects.filter(car_id=car_id)
...
class LocationCreateAPIView(APIView):
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            if Location.objects.filter(car=car, name=name).exists():
                return Response(
            ...
            return Response({"msg": "Location created"}, status=status.HTTP_201_CREATED)
        print(serializer.errors)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    def get(self, request, car_id=None):
        if car_id is not None:
            queryset = Location.objects.filter(id=car_id)
            serializer = LocationSerializer(queryset)
            return Response(serializer.data)
        return Response({"msg": "Slot not found"}, status=status.HTTP_404_NOT_FOUND)


class GetCarLocation(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request, car_id):
        try:
            locations = Location.objects.filter(car=car_id)
...
class SingleCarLocationDetailView(RetrieveAPIView):
    serializer_class = LocationSerializer
